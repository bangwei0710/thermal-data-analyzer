...
if uploaded_file is not None:
    try:
        ...
        # 強制指定 X 軸為 "time"，同時顯示總筆數
        if "time" not in df.columns:
            st.error("❗ 檔案中找不到 'time' 欄位，請確認欄位名稱是否為小寫 'time'")
        else:
            df["time"] = pd.to_datetime(df["time"], errors='coerce')
            st.info(f"📈 資料總筆數：{len(df)}")

            # 選擇 Y 軸欄位（不設數量限制）
            selected_cols = st.multiselect("📋 選擇要分析的欄位（1 個以上）", [col for col in all_columns if col != "time"])
            ...
            if selected_cols:
                ...
                df_subset = df_subset[["time"] + selected_cols].dropna()
                ...
                if df_subset.shape[0] < 2:
                    ...
                    ax1.plot(df_subset["time"], df_subset[selected_cols[0]], color='blue', label=selected_cols[0])
                    ax1.set_ylabel(selected_cols[0], color='blue')
                    ax1.tick_params(axis='y', labelcolor='blue')

                    if len(selected_cols) >= 2:
                        ax2 = ax1.twinx()
                        ax2.plot(df_subset["time"], df_subset[selected_cols[1]], color='green', label=selected_cols[1])
