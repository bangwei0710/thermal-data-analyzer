...
st.title("📊 Thermal Log 分析工具（Version 29e：整合所有檔案平均值 + 單列顯示）")
...
if uploaded_files:
    ...
    for col in common_params:
        match = [c for c in tail_df.columns if normalize(c) == normalize(col)]
        if match:
            values = pd.to_numeric(tail_df[match[0]], errors='coerce').dropna()
