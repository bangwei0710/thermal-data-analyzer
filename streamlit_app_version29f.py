...
st.title("📊 Thermal Log 分析工具（Version 29f：修正 normalize 對應）")
...
if uploaded_files:
    ...
    st.markdown("---")
    st.subheader("📌 常用參數快速分析（每檔各自 600 筆平均）")

    results = []

    for shortname, df in valid_dataframes:
        df_tail = df.tail(600)
        norm_map = {normalize(c): c for c in df_tail.columns}
        row = {"檔名": shortname}
        for col in common_params:
            norm_key = normalize(col)
            if norm_key in norm_map:
                real_col = norm_map[norm_key]
                values = pd.to_numeric(df_tail[real_col], errors='coerce').dropna()
    ...
    st.dataframe(summary_df.set_index("檔名"))

    st.markdown("---")
    st.subheader("📌 所有檔案合併後的常用參數平均值（最後 600 筆）")

    combined_df = pd.concat([df for _, df in valid_dataframes], ignore_index=True)
    tail_df = combined_df.tail(600)
    norm_map_all = {normalize(c): c for c in tail_df.columns}
    avg_result = {}

    for col in common_params:
        norm_key = normalize(col)
        if norm_key in norm_map_all:
            real_col = norm_map_all[norm_key]
            values = pd.to_numeric(tail_df[real_col], errors='coerce').dropna()
            avg_result[col] = f"{values.mean():.2f}" if not values.empty else "-"
        else:
            avg_result[col] = "-"

    vertical_df = pd.DataFrame(avg_result.items(), columns=["參數", "平均值"])
    st.dataframe(vertical_df.set_index("參數"))
