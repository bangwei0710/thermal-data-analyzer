...
st.title("📊 Thermal Log 分析工具（Version 32：常用參數直式顯示 + 總整表格）")
...
if uploaded_files:
    ...
    # 整合常用參數直式顯示 + 統整表格
    st.markdown("---")
    st.subheader("📌 常用參數快速分析（每檔個別直式 + 全部統整表格）")
    ...
    for shortname, df in valid_dataframes:
        ...
        for col in common_params:
            ...
            if match:
                ...
                value = "-"
            row[col] = value
            merged_list.append({"檔名": shortname, "參數名稱": col, "數值": value})
        results.append(row)
