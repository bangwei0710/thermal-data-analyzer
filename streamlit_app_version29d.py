...
st.title("📊 Thermal Log 分析工具（Version 29d：常用參數直式兩欄呈現）")
...
if uploaded_files:
    ...
    # 整合所有資料進行常用參數分析（直式表格顯示）
    st.markdown("---")
    st.subheader("📌 常用參數快速分析（所有資料合併，最後 600 筆）")
    ...
    summary_df = pd.DataFrame(result.items(), columns=["參數", "平均值"])
    st.dataframe(summary_df.set_index("參數"))
