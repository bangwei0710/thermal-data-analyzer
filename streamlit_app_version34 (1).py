...
valid_dataframes = []
export_raw_dataframes = []  # 收集所有被選擇的 raw data 區塊
    ...
    for uploaded_file in uploaded_files:
        try:
            ...
            if selected_cols:
                ...
                # 加入輸出列表：加上檔名前綴
                renamed = df_subset[selected_cols].copy()
                renamed.columns = [f"{shortname}::{col}" for col in renamed.columns]
                export_raw_dataframes.append(renamed.reset_index(drop=True))
